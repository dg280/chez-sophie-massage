...
${calendarButton(calendarLink({ date: payload.dt, heure: payload.h, soin: payload.s, duree: payload.d }))}
${cancelButton(cancelUrl(`https://${req.headers.host || 'www.sophie-tuina.fr'}`, { date: payload.dt, heure: payload.h, client: `${payload.pr} ${payload.nm}`.trim(), email: payload.em, soin: payload.s }, process.env.BOOKING_SECRET))}
